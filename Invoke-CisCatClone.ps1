...
Function Get-MAPS {
    ...
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "LocalSettingOverrideSpynetReporting"

    # Attempt to get the setting value
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    # Check the setting status and print the result
    if ($null -eq $settingValue) {
        Write-Output "18.9.47.3.1 (L1) Ensure 'Configure local setting override for reporting to Microsoft MAPS' is set to 'Disabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.3.1 (L1) Ensure 'Configure local setting override for reporting to Microsoft MAPS' is set to 'Disabled': **pass**"
    } else {
        Write-Output "18.9.47.3.1 (L1) Ensure 'Configure local setting override for reporting to Microsoft MAPS' is set to 'Disabled': **fail**"
    }
}

function Get-NetworkFiltering {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "EnableNetworkProtection"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.4.3.1 (L1) Ensure 'Prevent users and apps from accessing dangerous websites' is set to 'Enabled: Block': **not set**"
    } elseif ($settingValue -eq 1) {
        Write-Output "18.9.47.4.3.1 (L1) Ensure 'Prevent users and apps from accessing dangerous websites' is set to 'Enabled: Block': **pass**"
    } else {
        Write-Output "18.9.47.4.3.1 (L1) Ensure 'Prevent users and apps from accessing dangerous websites' is set to 'Enabled: Block': **fail**"
    }
}

function Get-ScanDownloadedFiles {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "DisableIOAVProtection"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.8.1 (L1) Ensure 'Scan all downloaded files and attachments' is set to 'Enabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.8.1 (L1) Ensure 'Scan all downloaded files and attachments' is set to 'Enabled': **pass**"
    } else {
        Write-Output "18.9.47.8.1 (L1) Ensure 'Scan all downloaded files and attachments' is set to 'Enabled': **fail**"
    }
}

function Get-RealtimeProtection {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "DisableRealtimeMonitoring"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.8.2 (L1) Ensure 'Turn off real-time protection' is set to 'Disabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.8.2 (L1) Ensure 'Turn off real-time protection' is set to 'Disabled': **pass**"
    } else {
        Write-Output "18.9.47.8.2 (L1) Ensure 'Turn off real-time protection' is set to 'Disabled': **fail**"
    }
}

function Get-BehaviorMonitoring {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "DisableBehaviorMonitoring"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.8.3 (L1) Ensure 'Turn on behavior monitoring' is set to 'Enabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.8.3 (L1) Ensure 'Turn on behavior monitoring' is set to 'Enabled': **pass**"
    } else {
        Write-Output "18.9.47.8.3 (L1) Ensure 'Turn on behavior monitoring' is set to 'Enabled': **fail**"
    }
}

function Get-ScanRemovableDrives {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "DisableRemovableDriveScanning"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.11.1 (L1) Ensure 'Scan removable drives' is set to 'Enabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.11.1 (L1) Ensure 'Scan removable drives' is set to 'Enabled': **pass**"
    } else {
        Write-Output "18.9.47.11.1 (L1) Ensure 'Scan removable drives' is set to 'Enabled': **fail**"
    }
}


function Get-ScanEmail {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "DisableEmailScanning"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.11.2 (L1) Ensure 'Turn on e-mail scanning' is set to 'Enabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.11.2 (L1) Ensure 'Turn on e-mail scanning' is set to 'Enabled': **pass**"
    } else {
        Write-Output "18.9.47.11.2 (L1) Ensure 'Turn on e-mail scanning' is set to 'Enabled': **fail**"
    }
}

function Get-PUAProtection {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "PUAProtection"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.14 (L1) Ensure 'Configure detection for potentially unwanted applications' is set to 'Enabled: Block': **not set**"
    } elseif ($settingValue -eq 1) {
        Write-Output "18.9.47.14 (L1) Ensure 'Configure detection for potentially unwanted applications' is set to 'Enabled: Block': **pass**"
    } else {
        Write-Output "18.9.47.14 (L1) Ensure 'Configure detection for potentially unwanted applications' is set to 'Enabled: Block': **fail**"
    }
}

function Get-DefenderAV {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Defender"
    $settingValueName = "DisableAntiSpyware"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.47.15 (L1) Ensure 'Turn off Microsoft Defender AntiVirus' is set to 'Disabled': **not set**"
    } elseif ($settingValue -eq 0) {
        Write-Output "18.9.47.15 (L1) Ensure 'Turn off Microsoft Defender AntiVirus' is set to 'Disabled': **pass**"
    } else {
        Write-Output "18.9.47.15 (L1) Ensure 'Turn off Microsoft Defender AntiVirus' is set to 'Disabled': **fail**"
    }
}
...
function Get-RDPPasswords {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\RemoteDesktopServices"
    $settingValueName = "DisablePasswordSaving"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "18.9.65.2.2 (L1) Ensure 'Do not allow passwords to be saved' is set to 'Enabled': **not set**"
    } elseif ($settingValue -eq 1) {
        Write-Output "18.9.65.2.2 (L1) Ensure 'Do not allow passwords to be saved' is set to 'Enabled': **pass**"
    } else {
        Write-Output "18.9.65.2.2 (L1) Ensure 'Do not allow passwords to be saved' is set to 'Enabled': **fail**"
    }
}
...
function Get-PasswordHistory {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\Password"
    $settingValueName = "DevicePasswordHistory"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "1.1.1 (L1) Ensure 'Enforce password history' is set to '24 or more passwords': **not set**"
    } elseif ($settingValue -ge 24) {
        Write-Output "1.1.1 (L1) Ensure 'Enforce password history' is set to '24 or more passwords': **pass**"
    } else {
        Write-Output "1.1.1 (L1) Ensure 'Enforce password history' is set to '24 or more passwords': **fail**"
    }
}

function Get-PasswordMaximumAge {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\DeviceLock"
    $settingValueName = "DevicePasswordExpiration"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "1.1.2 (L1) Ensure 'Maximum password age' is set to '365 or fewer days, but not 0': **not set**"
    } elseif ($settingValue -le 365 -and $settingValue -ne 0) {
        Write-Output "1.1.2 (L1) Ensure 'Maximum password age' is set to '365 or fewer days, but not 0': **pass**"
    } else {
        Write-Output "1.1.2 (L1) Ensure 'Maximum password age' is set to '365 or fewer days, but not 0': **fail**"
    }
}

function Get-PasswordMinimumAge {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\DeviceLock"
    $settingValueName = "MinimumPasswordAge"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "1.1.3 (L1) Ensure 'Minimum password age' is set to '1 or more day(s)': **not set**"
    } elseif ($settingValue -ge 1) {
        Write-Output "1.1.3 (L1) Ensure 'Minimum password age' is set to '1 or more day(s)': **pass**"
    } else {
        Write-Output "1.1.3 (L1) Ensure 'Minimum password age' is set to '1 or more day(s)': **fail**"
    }
}

function Get-PasswordComplexity {
    $settingPath = "HKLM:\SOFTWARE\Microsoft\PolicyManager\current\device\DeviceLock"
    $settingValueName = "PasswordComplexity"
    $settingValue = Get-ItemPropertyValue -Path $settingPath -Name $settingValueName -ErrorAction SilentlyContinue

    if ($null -eq $settingValue) {
        Write-Output "1.1.5 (L1) Ensure 'Password must meet complexity requirements' is set to 'Numbers, lowercase, uppercase and special characters required': **not set**"
    } elseif ($settingValue -eq 1) {
        Write-Output "1.1.5 (L1) Ensure 'Password must meet complexity requirements' is set to 'Numbers, lowercase, uppercase and special characters required': **pass**"
    } else {
        Write-Output "1.1.5 (L1) Ensure 'Password must meet complexity requirements' is set to 'Numbers, lowercase, uppercase and special characters required': **fail**"
    }
}
